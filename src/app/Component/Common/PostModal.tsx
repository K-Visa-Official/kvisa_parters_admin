  ...
  return (
    <div className={styles.modalBackdrop}>
      {la === "ko" || la === "ch" ?    
        <> 
          {modal_post ?
            <PostWork n = {pk} la={la} onClose={() => onClose()}/>
            :
            ...
            <div className={styles.modaluser}>
              ...
              <div className={styles.modalcard}>
                <img src=
                  {user.length !== 0 ? user[0]?.user?.bu_logo : 
                    user_no ? user_no?.bu_logo :  
                    "/common/ic_nonprofile.svg"}
                  style={{ width: "24px", height: "24px", marginLeft: "15px" }}
                />
                <div style={{ marginLeft: "20px" }}>
                  {user.length !== 0 ? user[0]?.user?.bu_name : user_no ? user_no?.bu_name :   ""}<br />
                  <span style={{ color: "#84848f", fontSize: "12px", marginTop: "2px" }}>{user.length !== 0 ? user[0]?.user?.bu_intro : user_no ? user_no?.bu_intro :  ""}</span>
                </div>
              </div>
